...
if [ $# -ne 2 ] ; then
    ...
    if [ $# -ne 6 ] ; then
        ...
        echo "            [FULL path to the folder with raw T2w stacks, e.g., /home/data/test]"
...
#    ${mirtk_path}/mirtk reconstructFFD ../SVR-output-${recon_roi}.nii.gz ${number_of_stacks} ../proc-stacks-${recon_roi}/*.nii*  -mask ../average_mask_cnn.nii.gz -template ../transf-selected_template.nii.gz -default_thickness ${default_thickness} -resolution ${recon_resolution} -default -dilation 4 -compensate
#
    ${mirtk_path}/mirtk reconstruct tmp-output.nii.gz  ${number_of_stacks} ../proc-stacks-${recon_roi}/*.nii* -mask ../average_mask_cnn.nii.gz -template ../transf-selected_template.nii.gz -default_thickness ${default_thickness} -svr_only -iterations 1 -resolution 1.6 ;
#

    if [ $motion_correction_mode -eq 1 ]; then
        ${mirtk_path}/mirtk reconstruct ../SVR-output-${recon_roi}.nii.gz ${number_of_stacks} ../proc-stacks-${recon_roi}/*.nii* -mask ../average_mask_cnn.nii.gz -template tmp-output.nii.gz -default_thickness ${default_thickness} -iterations 3 -resolution ${recon_resolution} -structural -svr_only
    else
    
    ${mirtk_path}/mirtk reconstruct ../SVR-output-${recon_roi}.nii.gz ${number_of_stacks} ../proc-stacks-${recon_roi}/*.nii* -mask ../average_mask_cnn.nii.gz -template tmp-output.nii.gz -default_thickness ${default_thickness}  -iterations 3 -resolution ${recon_resolution} -svr_only
    fi
...
#   -delta 110 -lambda 0.018 -lastIter 0.008 ;
    ...
    ${mirtk_path}/mirtk prepare-for-monai res-svr-files/ svr-files/ reo-svr-info.json reo-svr-info.csv ${res} ${number_of_stacks} SVR-output-${recon_roi}.nii.gz > tmp.log
    ...
    ${mirtk_path}/mirtk init-dof init.dof
    ${mirtk_path}/mirtk register-landmarks ${template_path}/brain-ref-atlas-2022/mask-${z1}.nii.gz ${all_org_stacks[$j]} init.dof dof-to-atl-${recon_roi}.dof ${n_roi} ${n_roi}  ${template_path}/brain-ref-atlas-2022/mask-${z1}.nii.gz ${template_path}/brain-ref-atlas-2022/mask-${z2}.nii.gz ${template_path}/brain-ref-atlas-2022/mask-${z3}.nii.gz ${template_path}/brain-ref-atlas-2022/mask-${z1}.nii.gz out-svr-reo-masks/mask-${z1}.nii.gz out-svr-reo-masks/mask-${z2}.nii.gz out-svr-reo-masks/mask-${z3}.nii.gz out-svr-reo-masks/mask-${z4}.nii.gz > tmp.log
